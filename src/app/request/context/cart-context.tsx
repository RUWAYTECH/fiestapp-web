...
export const CartProvider = ({ children }: CartProviderProps) => {
  ...
  return (
    <CartContext.Provider value={{ cart, addToCart, clearCart, handleRouteChange }}>
      {children}
      ...
      {showConfirmModal && (
        <div className="fixed inset-0 flex items-center justify-center bg-black/50 z-50">
          <div className="bg-white p-5 rounded-lg shadow-lg w-96">
            ...
            <p>Si sales de la pantalla de servicios, se perderán los productos.</p>
            <div className="mt-4 flex justify-end gap-2">
              <button className="bg-gray-300 px-4 py-2 rounded" onClick={() => confirmClearCart(false)}>
                Cancelar
              </button>
              <button className="bg-red-500 text-white px-4 py-2 rounded" onClick={() => confirmClearCart(true)}>
                Vaciar y salir
              </button>
            </div>
          </div>
